...
/**
 * Sonnet, still — but the reason to look again is written down here.
 *
 * Measured 2026-09-04, the first day token usage was logged: a batch of 34
 * products cost \$0.035, and **output tokens exceeded input** (1,983 against
 * 1,898). Output is 5x the price of input, so more than half the bill was the
 * `reasoning` prose — which the review screen truncates to two lines anyway.
 * Capping it to ten words is the cheap half of the fix.
 *
 * The other half is this line. `claude-haiku-4-5` costs \$5 per Mtok of output
 * against \$15, so the same workload is roughly a third. It is not a free swap:
 * placing a product in a store's own aisles is the app's one piece of judgement,
 * and a worse guess costs a person a walk across a shop. Change it against a
 * real store's layout and compare placements, not in the abstract — and do it in
 * the same pass as the Bedrock migration, which rewrites this same line.
 */
